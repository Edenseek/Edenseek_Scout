...
**Status:** ACCEPTED (posted by the Edenseek Publisher/Platform session for the Scout session to ratify,
place as the canonical Scout ADR in `Edenseek_Scout/docs/architecture/`, and commit — that commit is the
architecture-freeze point). **Date:** 2026-07-28. **Deciders:** founder; Publisher/Platform session; Scout
session (via the `publisher_bridge`). **Scope:** the Scout (`edenseek-scout`) repository's architecture as
it generalizes from single-issue Production Alpha to publisher-wide observability. Does NOT modify the
frozen Publisher platform.
...
## Instructions for the Scout session
1. **Ratify + adopt:** review this ADR; if you concur, place it as the canonical Scout ADR at
   `Edenseek_Scout/docs/architecture/ADR-0001-scout-publisher-observability-architecture.md` and commit it
   in the Scout repo. That commit is the architecture-freeze point (founder's condition).
2. **Then implement incrementally per D7** — IssueContext → Registry (seeded with the one certified issue) →
   Discovery → scheduler-from-Registry — keeping the certified single-issue production audit path intact at
   every step, **after** the Publisher completes 6.4 + close-out.
3. **No Publisher-side dependency to start:** everything is read-only against the certified contract + the
   v5 grant. If implementation surfaces a genuine Publisher-side need (per §Sequencing), raise it here on
   the bridge and it goes through Publisher governance — do not assume or build a Publisher change.
4. Reply on the bridge once ratified/committed so the record is closed on both sides.
